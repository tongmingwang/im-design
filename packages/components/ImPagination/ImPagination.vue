<template>
  <div
    ...
    <!-- total -->
    <div :class="[bem.e('total')]" v-if="layout.includes('total')">
      共 {{ total }} 条数据
    </div>
    <!-- prev -->
    <div
      :class="[bem.e('prev'), bem.is('disabled', isPrevDisabled)]"
      :aria-disabled="isPrevDisabled"
      v-if="layout.includes('prev')"
      v-ripple="!isPrevDisabled"
      @click="onPrevPage">
    ...
    <div :class="[bem.e('pager')]" v-if="layout.includes('pager')">
      <template v-for="item in pageList">
        ...
        <div
          v-else-if="item === 'right'"
          :class="[bem.e('page-item'), bem.e('right')]"
          title="向后5页"
          @click="onNextGroup">
          <ImIcon name="ellipsis" class="im-icon--ellipsis" />
    ...
    <!-- next -->
    <div
      :class="[bem.e('next'), bem.is('disabled', isNextDisabled)]"
      :aria-disabled="isNextDisabled"
      v-if="layout.includes('next')"
      v-ripple="!isNextDisabled"
      @click="onNextPage">
...
const emit = defineEmits<{
  (e: 'change', pageData: { pageNumber: number; pageSize: number }): void;
}>();
...
const totalPage = computed(() =>
  Math.ceil(props.total / (sizeValue.value || 10))
);
// 是否禁用上一页
...
const sizeOptions = computed(() =>
  props.pageSizeItems.map((item) => ({ label: `${item} 条/页`, value: item }))
);
...
watch(
  ...
  () => {
    ...
    updatePager(
      totalPage.value < props.pageNumber ? totalPage.value : props.pageNumber
    );
  },
  { immediate: true }
);
